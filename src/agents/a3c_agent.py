...
class A3CAgent:
    ...
    def learn(
        ...
        Returns:
            dict: métricas de treinamento (policy_loss, value_loss, entropy_loss, advantage, explained_variance)
        """
        try:
            # Calcula o retorno (G)
            R = 0
            ...
            # Calcular explained variance
            values_np = values_new.squeeze().detach().cpu().numpy()
            returns_np = returns.detach().cpu().numpy()
            var_y = np.var(returns_np)
            explained_variance = 1 - np.var(returns_np - values_np) / (var_y + 1e-8)
