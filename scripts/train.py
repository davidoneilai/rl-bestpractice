...
def train_dqn(config):
    ...
    for i_episode in range(1, config["num_episodes"] + 1):
        ...
        for t in range(config["max_steps_per_episode"]):
            ...
            # Capturar a loss do aprendizado
        ...
        logger.log_scalar("Episode Duration", t + 1, i_episode)
        logger.log_scalar("Epsilon", epsilon, i_episode)
        
        # Registrar loss média do episódio se houve aprendizado
        if episode_losses:
            logger.log_scalar("Loss", np.mean(episode_losses), i_episode)
    ...
    try:
        ...
        while episode_counter.value < config["max_episodes"]:
            try:
                ...
                # Capturar métricas do aprendizado
                metrics = agent.learn(
                    states,
                    actions,
                    rewards,
                ...
                with episode_counter.get_lock():
                    ...
                    logger.log_scalar("Global/Episode_Duration", episode_step, current_episode)
                    
                    # Registrar métricas de aprendizado A3C
                    if metrics:
                        logger.log_scalar(f"Worker_{worker_id}/Policy_Loss", metrics['policy_loss'], current_episode)
                        logger.log_scalar(f"Worker_{worker_id}/Value_Loss", metrics['value_loss'], current_episode)
                        logger.log_scalar(f"Worker_{worker_id}/Entropy_Loss", metrics['entropy_loss'], current_episode)
                        logger.log_scalar(f"Worker_{worker_id}/Advantage", metrics['advantage'], current_episode)
                        logger.log_scalar(f"Worker_{worker_id}/Explained_Variance", metrics['explained_variance'], current_episode)
                        
                        # Métricas globais (só do worker 0 para evitar duplicação)
                        if worker_id == 0:
                            logger.log_scalar("Global/Policy_Loss", metrics['policy_loss'], current_episode)
                            logger.log_scalar("Global/Value_Loss", metrics['value_loss'], current_episode)
                            logger.log_scalar("Global/Entropy_Loss", metrics['entropy_loss'], current_episode)
                            logger.log_scalar("Global/Advantage", metrics['advantage'], current_episode)
                            logger.log_scalar("Global/Explained_Variance", metrics['explained_variance'], current_episode)
